...
export default function AuctionDetailPage() {
  ...
  const [pendingOferta, setPendingOferta] = useState<Bid | null>(null);
  ...
  const handleConfirmFinalize = async () => {
    if (!auction || !pendingOferta) return;
    setFinalizing(true);
    setFinalizeError(null);
    try {
      await endAuction(auction.id, pendingOferta.bidId);
      setPendingOferta(null);
      const updated = await getAuctionById(auction.id);
      setAuction(updated);
    } catch {
            ...
            {auction.bids
              ...
              .map((o: Bid) => (
                <OfertaRow key={o.bidId} $estado={o.status}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600 }}>{o.postor.name}</div>
                    <div style={{ fontSize: '0.85rem', color: theme.colors.textSecondary, marginTop: '0.2rem' }}>
                      {'★'.repeat(Math.round(o.postor.rating))} ({o.postor.rating.toFixed(1)})
                    </div>
          ...
          <ConfirmModal>
            ...
            <ExchangeSummary>
              ...
              <div>
                ...
                <div className="value">
                  {'★'.repeat(Math.round(pendingOferta.postor.rating))} ({pendingOferta.postor.rating.toFixed(1)})
                </div>
              </div>
            </ExchangeSummary>

            <ConfirmFooter>
              <CancelBtn onClick={() => setPendingOferta(null)} disabled={finalizing}>
                Cancelar
              </CancelBtn>
